...
| `APP_BASE_URL` | no | _(empty)_ | Public console URL (e.g. `https://signals.example.com`). When set, signal/digest emails link back to the Signal detail page. |
| `DIGEST_TICK_SECONDS` | no | `60` | How often the digest scheduler checks for due email digests. See [EMAIL.md](EMAIL.md). |

...
func TestDBErrorPaths(t *testing.T) {
	...
	_, err = d.ListSignals(ctx, db.SignalFilter{Industry: ptrS("X"), GeoScope: ptrS("LOCAL"), Sentiment: ptrS("NEGATIVE"), Influence: ptrS("HIGH"), Region: ptrS("R"), MinRelevance: ptrF(0.5)})
	mustErr("ListSignals(attr filters)", err)
